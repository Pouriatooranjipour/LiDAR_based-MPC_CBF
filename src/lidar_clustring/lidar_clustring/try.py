...
class LidarClusteringNode(Node):
    ...
    def scan_callback(self, msg):
        ...
        x = ranges * np.cos(angles-pi)
        y = ranges * np.sin(angles-pi)
        
        # Create a PointStamped in the Lidar frame
        obstacle_point_lidar = PointStamped()
        obstacle_point_lidar.header.stamp = self.get_clock().now().to_msg()
        obstacle_point_lidar.header.frame_id = 'laser'
        obstacle_point_lidar.point.x = x
        obstacle_point_lidar.point.y = y
        obstacle_point_lidar.point.z = 0.0
        
        try:
            # Look up the transform from Lidar frame to Odom frame
            transform = self.tf_buffer.lookup_transform(
                'odom',
                'laser',
                obstacle_point_lidar.header.stamp
            )
            ...
            self.odom_publisher.publish(obstacle_point_odom)
